...
function RichEditor({ value, onChange }) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "prose prose-sm max-w-none min-h-[140px] p-3 focus:outline-none text-slate-800 leading-relaxed",
      },
    },
  })
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            {m.replied === "false" ? (
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-red-100 text-red-600 font-medium">Pending</span>
            ) : (
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-600 font-medium">Replied</span>
            )}
          </div>
        ...
        {m.subject && (
          <div className="text-sm text-slate-700 font-medium truncate mt-1">{m.subject}</div>
        )}
...
export default function Messages() {
  ...
  async function handleSendReply() {
    if (!reply || reply === "<p></p>") return error("Write something first")
    setSending(true)
    try {
      await sendReply({
        id: selected.id,
      ...
      {selected && (
        ...
        >
          <div className="space-y-4">
            ...
            {selected.replied === "true" ? (
              <div>
                <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-1.5">Your Reply</label>
                <div
                  className="prose prose-sm max-w-none border border-slate-200 rounded-xl p-3 bg-slate-50 text-slate-700"
                  dangerouslySetInnerHTML={{ __html: selected.reply || "<p>No reply found</p>" }}
                />
              </div>
            ) : (
              <div>
